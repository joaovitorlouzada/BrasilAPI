...
function BrokerCatalog() {
  ...
  // Flag para saber se o usuário está filtrando
  const hasActiveFilter = searchString.trim().length > 0;
    ...
    <div className="broker-catalog">
      ...
      {!error && (
        <>
          ...
          <p>
            Total de corretoras na base:{" "}
            <strong>{totalLoaded}</strong>
            {hasActiveFilter && (
              <>
                {" "}
                | Corretoras que atendem aos filtros:{" "}
                <strong>{brokersParaExibir.length}</strong>
              </>
            )}
          </p>

          {searchString.trim() &&
            brokersParaExibir.length === 0 &&
            !loading && (
              <p
                className="error"
                style={{ border: "none", backgroundColor: "transparent" }}
              >
                Nenhum resultado encontrado para "{searchString}".
              </p>
            )}
